...
func OptionLaunchVirtualMachineFromUsableQCOW2(
    ...
    extraArgs, err = _addSharePathArgs(extraArgs, sharePath)
    if err != nil {
        return fmt.Errorf("adding args share path: %w", err)
    }
    extraArgs, err = _addSpiceArgs(extraArgs)
    if err != nil {
...
func _addSharePathArgs(
    extraArgs string,
    sharePath *pathlib.Path,
) (string, error) {
    return _addExtraArgs(
        fmt.Sprintf("Share %v with virtual machine?", sharePath),
        extraArgs,
        fmt.Sprintf("-virtfs local,path=%v,mount_tag=host0,"+
        "security_model=mapped,id=host0", sharePath))
}

func _addSpiceArgs(extraArgs string) (string, error) {
    return _addExtraArgs(
        "Run with spice?",
        extraArgs,
        strings.Join(
            []string{ "-spice port=3001,disable-ticketing=on",
...
func _launchVM(diskPath *pathlib.Path, extraArgs string) (error) {
    ...
    log.WithField("qemuRunCmd", qemuRunCmd).
        Info("Running qemu with the following command")
